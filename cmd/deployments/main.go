...
func main() {
	...
	//clientset.AppsV1().ReplicaSets("nginx-ingress").List(context.Background(), metav1.ListOptions{})
	...
	deployments, err := clientset.AppsV1().Deployments("nginx-ingress").Get(context.Background(), "nginx-ingress", metav1.GetOptions{})
	if err != nil {
		panic(err.Error())
	}
	//fmt.Printf("%+v\n", deployments.Name)

	rep := deployments.Spec.Replicas
	var r int32 = *rep
	...
	// clientset.CoreV1().Pods("nginx-ingress")

	// rs, err := clientset.CoreV1().Pods("nginx-ingress").List(context.Background(), metav1.ListOptions{
	// 	FieldSelector: "",
	// })
	// if err != nil {
	// 	panic(err.Error())
	...
	p, err := clientset.CoreV1().Pods("nginx-ingress").Get(context.Background(), "nginx-ingress-9b9b8f76b-fs8sh", metav1.GetOptions{})
	if err != nil {
		panic(err)
	}
